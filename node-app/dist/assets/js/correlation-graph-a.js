...
function render(props) {
  ...
  var parent = document.getElementById('corr_graph_a');
  ...
  var corr_svg_a = d3.select(selector).append('svg').attr('width', width).attr('height', height);

  var backgroundRect = corr_svg_a.append('rect').attr('width', width).attr('height', height).classed('background', true).style('fill', 'white');
  ...
  var link = corr_svg_a.append('g').style('stroke', '#aaa').selectAll('line').data(links).enter().append('line').style('stroke-width', function (d) {
    return linkWidthScale(d.weight);
  }).style('stroke-opacity', 0.4);

  link.attr('class', 'link').attr('marker-end', 'url(#end-arrow)');

  var nodesParentG = corr_svg_a.append('g').attr('class', 'nodes');
